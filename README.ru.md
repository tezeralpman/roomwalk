...
Отдельный файл [`skills/roomwalk/higgsfield.md`](plugins/roomwalk/skills/roomwalk/higgsfield.md)
— про генерацию, а не про вёрстку: готовые пресеты движения, замеренные цены,
порядок операций. Коротко, что там важнее всего:

**Не генерируйте в 4K.** Десять секунд в 4K — 220 кредитов, в 1080p — 90, а
апскейл до 4K с частотой 48 кадров — 8. Итого 98 против 220, и вдобавок вдвое
больше кадров, потому что апскейлер доинтерполирует частоту. Для промотки
количество кадров и есть плавность.

**Смотрите пресеты раньше промптов.** `ORBIT 360` обводит камеру вокруг человека
на полный круг с замороженной позой — петля замыкается по построению, её не надо
выпрашивать словами. `ANDROID ASSEMBLE` собирает предмет из разлетевшихся
деталей: это `exploded` наоборот.

**Фон убирают, а не выравнивают.** Есть `sam_3_video`. Мерцание убирает
`video_deflicker`. Длину набирают режимом `video_extension`, а не склейкой.
